...
class HistoryPage extends Page {
    ...
    public function getCMSFields() {
		...
		$config2->addComponents(new GridFieldExportButton('before'));
		$config2->addComponent(new GridFieldSortableRows('SortOrder'));
		$toolbox = GridField::create('TopImages',false, $this->TopImages()->sort('SortOrder'), $config2);
		if(Permission::check("ADMIN")){
			$fields->addFieldToTab('Root.TopImages', $toolbox);
		}
...
class HistoryPage_Controller extends Page_Controller {
	...
	public function TopImage(){
		$d = HistoryPage::get()->First()->TopImages()->sort('RAND()')->First();
